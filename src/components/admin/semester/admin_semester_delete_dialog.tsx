...
import {DeleteSemesterAPI} from "../../../apis/semester_api.ts";
import {SemesterEntity} from "../../../models/entity/semester_entity.ts";

/**
 * # AdminSemesterDeleteDialog
 * > 用于显示一个删除学期的对话框，用户可以确认或取消删除操作。
 *
 * @param {SemesterEntity} semester - 要删除的学期实体。
 ...
 *
 * @returns {JSX.Element} 返回一个React组件，表示删除学期的对话框。
 */
export function AdminSemesterDeleteDialog({semester, show, emit, requestRefresh}: Readonly<{
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const getResp = await DeleteSemesterAPI(semester.semester_uuid);
    ...
    return (
        <Modal open={isModalOpen} onOk={handleOk} onCancel={handleCancel}
               footer={
                   <div className="modal-action">
                       <div className={"flex space-x-3"}>
                           ...
                           <button type={"submit"} form={"semester_remove"}
            ...
            <div className="flex flex-col space-y-4">
                ...
                <form id={"semester_remove"} onSubmit={onSubmit} className="py-2 grid space-y-2">
                    <div className="text-base-content flex space-x-0.5">
                        <span>是否确认删除</span>
                        <strong className={"text-primary font-bold"}>{semester.name}</strong>
                        <span>？</span>
                    </div>
