...
import {GetSemesterAPI, UpdateSemesterAPI} from "../../../apis/semester_api.ts";
import dayjs from "dayjs";
import { DatePicker } from "antd";

/**
 * # AdminSemesterEditDialog
 * > 该函数用于创建一个对话框，管理员可以通过这个对话框编辑已有的学期信息。此组件以模态对话框的形式呈现，并且提供了关闭对话框的功能。
 *
 * @param show - 一个布尔值，表示对话框是否显示。
 * @param emit - 一个函数，用于向父组件发送数据。
 * @param editSemesterUuid - 一个字符串，表示待编辑的学期的唯一标识符。
 * @param requestRefresh - 一个函数，用于请求刷新数据。
 * @returns {JSX.Element} 返回一个包含标题、表单和按钮的模态对话框组件。
 */
export function AdminSemesterEditDialog({show, emit, editSemesterUuid, requestRefresh}: Readonly<{
    show: boolean;
    editSemesterUuid: string;
    ...
    useEffect(() => {
        const func = async () => {
            if (!editSemesterUuid) return;
            
            const getResp = await GetSemesterAPI(editSemesterUuid);
            if (getResp?.output === "Success") {
                const semesterData = getResp.data!;
                setData({
                    name: semesterData.name,
                    description: semesterData.description || "",
                    start_date: semesterData.start_date,
                    end_date: semesterData.end_date,
                    is_enabled: semesterData.is_enabled
                } as SemesterDTO);
                
                // 设置日期选择器的值
                setStartDate(dayjs(semesterData.start_date));
                setEndDate(dayjs(semesterData.end_date));
            } else {
                message.error(getResp?.error_message || "获取学期信息失败");
            }
        }
    ...
    return (
        <Modal open={isModalOpen} onOk={handleOk} onCancel={handleCancel}
               footer={
                   <div className="modal-action">
                       <div className={"flex space-x-3"}>
                           ...
                           <button type={"submit"} form={"semester_edit"}
            ...
            <div className="flex flex-col space-y-4">
                ...
                <form id={"semester_edit"} onSubmit={onSubmit} className="py-2 grid space-y-2">
                    ...
                    <fieldset className="flex items-center space-x-3">
                        <input
                            ...
                            type="checkbox" className={"toggle toggle-sm"} checked={data.is_enabled}/>
                        <span>启用学期</span>
                    </fieldset>
                </form>
            </div>
