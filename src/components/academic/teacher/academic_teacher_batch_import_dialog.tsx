...
import {People, CheckOne, CloseOne, Download, Upload} from "@icon-park/react";
import {JSX, useEffect, useState} from "react";
import {message, Modal, Upload as AntUpload} from "antd";
import {RcFile} from "antd/es/upload";
import {UploadRequestOption} from "rc-upload/lib/interface";
import {GetTeacherTemplateAPI, BatchImportTeacherAPI} from "../../../apis/teacher_api.ts";
/**
 * # AcademicTeacherBatchImportDialog
 * > 该组件用于创建一个对话框，管理员可以通过这个对话框下载教师导入模板并批量导入教师信息。
 *
 ...
 * @constructor
 */
export function AcademicTeacherBatchImportDialog({show, emit, requestRefresh}: Readonly<{
    show: boolean;
    ...
    const downloadTemplate = async () => {
        try {
            const getResp = await GetTeacherTemplateAPI();
            ...
            const fileName = getResp.data.name || '教师导入模板.xlsx';
    ...
    const customRequest = async (options: UploadRequestOption) => {
        ...
        try {
            ...
            reader.onload = async () => {
                ...
                try {
                    const result = await BatchImportTeacherAPI(base64Content, ignoreError);
