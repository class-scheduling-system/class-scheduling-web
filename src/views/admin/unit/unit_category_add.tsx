...
import { AddUnitCategoryAPI } from "../../../apis/unit_category_api";

export function AdminUnitCategoryAdd({ site }: Readonly<{ site: SiteInfoEntity }>): JSX.Element {
    ...
    useEffect(() => {
        document.title = `添加单位类别 | ${site.name ?? "Frontleaves Technology"}`;
    }, [site.name]);
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await AddUnitCategoryAPI(data);
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="card card-border bg-base-100 w-full shadow-md">
                <h2 className="card-title bg-neutral/10 rounded-t-lg p-3">
                    <AddUser theme="outline" size="18" />添加单位类别信息
                </h2>
                <div className="card-body">
                    <form id="unit_category_add" onSubmit={onSubmit} className="flex flex-col space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {/* 单位类别简称 */}
                            <div className="form-control">
                                <label className="label" htmlFor="short_name" aria-label="单位类别简称">
                            ...
                            {/* 英文名称 */}
                            <div className="form-control">
                                <label className="label" htmlFor="english_name" aria-label="英文名称">
                                    <span className="label-text">
                                        <span>英文名称</span>
                                    </span>
                                </label>
                            ...
                            {/* 排序 */}
                            <div className="form-control">
                                <label className="label" htmlFor="order" aria-label="排序">
                                    <span className="label-text">
                                        <span>排序</span>
                                    </span>
                                </label>
